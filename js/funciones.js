...
function cargarFunciones(fechaSeleccionada = "") {
  ...
  const funciones = DB.funciones.filter((funcion) => {
    // Si no se seleccionó ninguna fecha, mostramos todas las funciones
    if (fechaSeleccionada === "") return true;

    // Si hay una fecha seleccionada, mostramos solo las funciones de ese día
  ...
  funciones.forEach((funcion) => {
    // Dependiendo del tipo de contenido, buscamos en películas o en obras
    const lista = funcion.tipo === "teatro" ? DB.obras : DB.peliculas;

    // Buscamos la película u obra correspondiente
    const contenido = lista.find((item) => item.id === funcion.contenidoId);

    // Buscamos la sala correspondiente
    const sala = DB.salas.find((item) => item.id === funcion.salaId);

    // Si falta algún dato, no mostramos esta función
    if (!contenido || !sala) return;
        ...
        <div class="row g-0 align-items-center">
          ...
          <div class="col-12 col-md-9">
            <div class="card-body">
              ...
              <h3 class="fw-bold">${contenido.titulo}</h3>
...
function configurarEventosFecha() {
  // Obtenemos todos los botones que tengan la clase .btn-fecha
  ...
  botones.forEach((btn) => {
    ...
    btn.addEventListener("click", () => {
      ...
      // Obtenemos la fecha almacenada en data-fecha
